...
tb  = data1['tb'].values
ygb = data1['ygb'].values
yg1 = data1['yg1_i'].values


# In[3]:

figure(1)
plot(tb,ygb,label = 'Base')
plot(tb,yg1,label = 'Mass 1')
xlabel('time (s)')
...
# plotspec(1024,delt,yg1[-1025:-1])  # Spectrum of just the response channel for the last 1024 data points,
        # corresponding to the free vibration after machine has been switched off
plotspec(2048,delt,ygb,yg1)  # Spectra of the first 2048 data points of both the excitation channel ygb
        #  and the response channel yg1
